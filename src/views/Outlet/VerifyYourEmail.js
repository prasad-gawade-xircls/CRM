...
const VerifyYourEmail = ({isQuick = false}) => {
          ...
          return (
            <>
              <div className='form-check form-switch form-check-primary mb-1 cursor-pointer'>
                <Input type='checkbox' id='verify' defaultChecked={emailList === row.email_id} onChange={(e) => emailStatusChange(e, row)} />
              </div>
            </>
          )
        }
        ...
        <Card>
          <CardBody>
            <div className='d-flex justify-content-center align-items-end gap-2'>
              ...
              {/* <input type='text' className='form-control' value={textValue} onChange={(e) => setTextValue(e.target.value)} /> */}
              <a style={{whiteSpace: 'nowrap', display: `${isQuick ? "none" : ""}`}} id="verify_btn_here" className='btn btn-primary' onClick={() => verifyEmail()}> Click to Get Verification mail </a>
            </div>
            {/* <p id="textValue_val" className="text-danger m-0 p-0 vaildMessage"></p> */}
            {
              data.length > 0 && !isQuick ? <>
                <Row className='mt-2'>
                    <ComTable
